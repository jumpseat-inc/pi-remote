...
Q3 became the seed of [[Spec Correction Governance]]: spec corrections forced by authoritative upstream evidence and preserving the security model are prose-sync within the implementing card's mandate. Also notable: the seat's grounding framework ("PETA SPKLU") originates from a different product and is explicitly disclaimed — see [[Council Seats]] for the provenance note. Meta-note: `ALREADY_LIVE_COPY` (EV-2 Item 3) and the EV-8 footer acceptance are treated as binding downstream text.
